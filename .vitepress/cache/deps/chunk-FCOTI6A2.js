...
function getIsIOS() {
  var _a, _b
  return isClient && ((_a = window == null ? void 0 : window.navigator) == null ? void 0 : _a.userAgent)
...
function useArrayIncludes(...args) {
  ...
  return computed(() =>
    toValue(list).slice(formIndex).some((element, index, array) =>
      comparator(
        toValue(element),
        toValue(value),
        index,
...
function useEventListener(...args) {
  ...
  const stopWatch = watchImmediate(
    ...
    ([raw_targets, raw_events, raw_listeners, raw_options]) => {
      cleanup()
      if (
        !(raw_targets == null ? void 0 : raw_targets.length) || !(raw_events == null ? void 0 : raw_events.length)
...
function onClickOutside(target, handler, options = {}) {
  const { window: window2 = defaultWindow, ignore = [], capture = true, detectIframe = false, controls = false } =
    options
  if (!window2) {
  ...
  const shouldIgnore = (event) => {
    return toValue(ignore).some((target2) => {
      if (typeof target2 === 'string') {
        return Array.from(window2.document.querySelectorAll(target2)).some((el) =>
  ...
  const stopWatch = watchEffect(() => {
    ...
    if (el) {
      const { stop } = useMutationObserver(
        document2,
        (mutationsList) => {
          const targetRemoved = mutationsList.map((mutation) => [...mutation.removedNodes]).flat().some((node) =>
...
function useBreakpoints(breakpoints, options = {}) {
  ...
  function current() {
    const points = Object.keys(breakpoints).map((k) => [k, shortcutMethods[k], pxValue(getValue2(k))]).sort((a, b) =>
      a[2] - b[2]
    )
  ...
  function dispatchWriteEvent(oldValue, newValue) {
    if (window2) {
      ...
      window2.dispatchEvent(
        storage instanceof Storage ? new StorageEvent('storage', payload) : new CustomEvent(customStorageEventName, {
          detail: payload,
        }),
      )
    }
...
function useColorMode(options = {}) {
  ...
  const updateHTMLAttrs = getSSRHandler(
    ...
    (selector2, attribute2, value) => {
      const el = typeof selector2 === 'string'
        ? window2 == null ? void 0 : window2.document.querySelector(selector2)
...
function useDeviceMotion(options = {}) {
  ...
  const requirePermissions = useSupported(() =>
    isSupported.value && 'requestPermission' in DeviceMotionEvent
...
function useDraggable(target, options = {}) {
  ...
  const start = (e) => {
    ...
    const containerRect = (_a2 = container == null ? void 0 : container.getBoundingClientRect) == null
      ? void 0
      : _a2.call(container)
...
function useElementByPoint(options) {
  ...
  const cb = () => {
    ...
    element.value = toValue(multiple)
      ? (_a = document2 == null ? void 0 : document2.elementsFromPoint(toValue(x), toValue(y))) != null ? _a : []
...
function useFetch(url, ...args) {
  ...
  const execute = async (throwOnFailed = false) => {
    ...
    if (payload) {
      ...
      if (
        !config.payloadType && payload && (proto === Object.prototype || Array.isArray(proto))
...
function useFocusWithin(target, options = {}) {
  ...
  useEventListener(targetElement, EVENT_FOCUS_OUT, () => {
    var _a, _b, _c
    return _focused.value = (_c = (_b = (_a = targetElement.value) == null ? void 0 : _a.matches) == null
        ? void 0
...
function useFullscreen(target, options = {}) {
  ...
  const isSupported = useSupported(() =>
    targetRef.value && document2 && requestMethod.value !== void 0 && exitMethod.value !== void 0
...
function useScroll(element, options = {}) {
  ...
  function scrollTo(_x, _y) {
    ...
    const scrollContainer = ((_d = _element == null ? void 0 : _element.document) == null ? void 0 : _d.documentElement)
      || (_element == null ? void 0 : _element.documentElement) || _element
    if (x != null) {
  ...
  const setArrivedState = (target) => {
    ...
    const el = ((_a = target == null ? void 0 : target.document) == null ? void 0 : _a.documentElement)
      || (target == null ? void 0 : target.documentElement) || unrefElement(target)
    ...
    const right = Math.abs(scrollLeft * directionMultipler) + el.clientWidth
...
function usePointerLock(target, options = {}) {
  ...
  async function lock(e) {
    ...
    targetElement = e instanceof Event
      ? (_a = unrefElement(target)) != null ? _a : triggerElement.value
      : unrefElement(e)
    if (!targetElement) {
...
function usePointerSwipe(target, options = {}) {
  ...
  const eventIsAllowed = (e) => {
    ...
    return (_c = (_b = (_a = options.pointerTypes) == null ? void 0 : _a.includes(e.pointerType)) != null
        ? _b
...
function useTextDirection(options = {}) {
  ...
  function getValue2() {
    var _a, _b
    return (_b = (_a = document2 == null ? void 0 : document2.querySelector(selector)) == null
        ? void 0
...
function useWakeLock(options = {}) {
  ...
  if (isSupported.value) {
    ...
    whenever(
      () =>
        documentVisibility.value === 'visible' && (document2 == null ? void 0 : document2.visibilityState) === 'visible'
        && requestedType.value,
      (type) => {
...
// # sourceMappingURL=chunk-FCOTI6A2.js.map
