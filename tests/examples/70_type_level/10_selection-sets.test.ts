...
test(`10_selection-sets`, async () => {
  const exampleResult = await runExample(`./examples/70_type_level/10_selection-sets.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../../examples/__outputs__/70_type_level/10_selection-sets.output.txt`,
  )
})
