  ...
  for (const key in fieldSelection.selectionSet) {
    ...
    switch (keyParsed.type) {
      ...
      default: {
        ...
        // After the above checks, sddm?.nt must be OutputObject or undefined
        const sddmOutputObject = sddm?.nt as SDDM.OutputObject | undefined
        collectForInlineFragmentLike(context, sddmOutputObject, keyParsed, {
          directives,
          selections,
        })
      }
