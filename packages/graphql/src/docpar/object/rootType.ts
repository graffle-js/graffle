 ...
 */
export const createStaticRootType = (operationType: Document.Ast.OperationTypeNode, options?: Options) => {
  return new Proxy({}, {
    get: (_, fieldName: string) => {
      ...
      if (fieldName === '$batch') {
        return (selection: Select.SelectionSet.AnySelectionSet, opts?: Partial<Options>) => {
          // Selection already contains multiple fields, no wrapping needed
          const documentNormalized = SelectDocument.createDocumentNormalizedFromRootTypeSelection(
            operationType,
            selection,
          )
