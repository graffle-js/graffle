...
test(`transport-http_raw`, async () => {
  const exampleResult = await runExampleForTest(`./examples/10_transport-http/transport-http_raw.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../__outputs__/10_transport-http/transport-http_raw.output.test.txt`,
  )
})
