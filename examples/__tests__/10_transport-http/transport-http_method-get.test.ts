...
test(`transport-http_method-get`, async () => {
  const exampleResult = await runExample(`./examples/10_transport-http/transport-http_method-get.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../__outputs__/10_transport-http/transport-http_method-get.output.txt`,
  )
})
