...
test(`document-builder_document`, async () => {
  const exampleResult = await runExampleForTest(`./examples/55_document-builder/document-builder_document.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../__outputs__/55_document-builder/document-builder_document.output.test.txt`,
  )
})
