...
test(`document-builder_batch`, async () => {
  const exampleResult = await runExampleForTest(`./examples/55_document-builder/document-builder_batch.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../__outputs__/55_document-builder/document-builder_batch.output.test.txt`,
  )
})
