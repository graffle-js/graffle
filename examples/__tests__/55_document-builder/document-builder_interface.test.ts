...
test(`document-builder_interface`, async () => {
  const exampleResult = await runExampleForTest(`./examples/55_document-builder/document-builder_interface.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../__outputs__/55_document-builder/document-builder_interface.output.test.txt`,
  )
})
