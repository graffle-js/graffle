...
test(`anyware_slot_slot-body__slot-search-params`, async () => {
  const exampleResult = await runExampleForTest(`./examples/50_anyware/anyware_slot_slot-body__slot-search-params.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../__outputs__/50_anyware/anyware_slot_slot-body__slot-search-params.output.test.txt`,
  )
})
