...
test(`anyware_short-circuit__short-circuit`, async () => {
  const exampleResult = await runExampleForTest(`./examples/50_anyware/anyware_short-circuit__short-circuit.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../__outputs__/50_anyware/anyware_short-circuit__short-circuit.output.test.txt`,
  )
})
