...
test(`output_envelope_envelope_error-throw__envelope-error-throw`, async () => {
  const exampleResult = await runExample(`./20_output/output_envelope_envelope_error-throw__envelope-error-throw.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../__outputs__/20_output/output_envelope_envelope_error-throw__envelope-error-throw.output.txt`,
  )
})
