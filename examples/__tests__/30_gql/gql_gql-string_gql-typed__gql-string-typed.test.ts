...
test(`gql_gql-string_gql-typed__gql-string-typed`, async () => {
  const exampleResult = await runExampleForTest(`./examples/30_gql/gql_gql-string_gql-typed__gql-string-typed.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../__outputs__/30_gql/gql_gql-string_gql-typed__gql-string-typed.output.test.txt`,
  )
})
