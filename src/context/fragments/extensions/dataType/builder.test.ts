...
describe(`creator`, () => {
  test(`creates a chain`, () => {
    ...
    Ts.Assert.exact.ofAs<Chain<Context, DataEmpty<'a'>>>().on(chain)
  })
})
...
describe(`properties`, () => {
  test(`can add static properties`, () => {
    ...
    // todo: readonly a: ...
    Ts.Assert.exact.ofAs<{ a: number }>().on(a.propertiesStatic)
  })
  test(`can add multiple static properties`, () => {
    const a = create(nameA).properties({ a: 1 }).properties({ b: 2 }).return()
    Ts.Assert.exact.ofAs<{ a: number } & { b: number }>().on(a.propertiesStatic)
  })
})
...
describe(`chain`, () => {
  test(`returns data if no configurator given`, () => {
    ...
    `)
    Ts.Assert.exact.ofAs<dataExpected>().on(data)
  })
})
...
describe(`creator`, () => {
  ...
  test(`input optional if configurator has no required properties`, () => {
    ...
    `)
    Ts.Assert.sub.ofAs<expectedCreator>().on(A)
    // rejects, why?
  ...
  test(`input -> returns with initial input`, () => {
    ...
    Ts.Assert.exact.ofAs<undefined>().on(a.configuratorInitialInput)

    const configuration = {}
    type configuration = typeof configuration
    const a2 = A(configuration)
    expect(a2.configuratorInitialInput).toBe(configuration)
    Ts.Assert.exact.ofAs<configuration>().on(a2.configuratorInitialInput)
  })
})
