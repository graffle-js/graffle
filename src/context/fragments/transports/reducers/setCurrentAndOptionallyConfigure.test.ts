...
const test = t.extend<{ c2: AddMany<ContextEmpty, [ATransport, BTransport]> }>({
  c2: async ({ c0 }, use) => {
    ...
    Assert.equiv.ofAs<ATransport['name']>().on(c2.transports.current)
    await use(c2)
  },
})
...
test(`can set current and configure it`, ({ c2 }) => {
  ...
  Assert.equiv.ofAs<BTransport['name']>().on(c3.transports.current)
  Assert.sub.ofAs<{ readonly BTransport: { b: string } }>().on(c3.transports.configurations)
})

test(`can set current and NOT configure it`, ({ c2 }) => {
  const c3 = setCurrentAndOptionallyConfigure(c2, BTransport.name)
  Assert.equiv.ofAs<BTransport['name']>().on(c3.transports.current)
  Assert.exact.ofAs<{}>().on(c3.transports.configurations.BTransport)
})
