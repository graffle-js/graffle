  ...
  describe('with defaults.errorChannel: "return"', () => {
    describe('.errors', () => {
      test('defaults to execution errors in envelope', () => {
        ...
        expectTypeOf(g.query.__typename()).resolves.toMatchTypeOf<ExecutionResult<{ __typename: 'Query' }> | RequestPipelineBase.ResultFailure>()
      })
      test('.execution:false restores errors to return', async () => {
        ...
        expectTypeOf(await g.query.__typename()).toEqualTypeOf<
          Omit<ExecutionResult<{ __typename: 'Query' }>, 'errors'> | RequestPipelineBase.ResultFailure | GraphQLExecutionResultError
        >()
      })
...
describe('defaults.errorChannel: "return"', () => {
  describe('puts errors into return type', () => {
    ...
    test('query.<fieldMethod>', async () => {
      expectTypeOf(await g.query.__typename()).toEqualTypeOf<
        'Query' | RequestPipelineBase.ResultFailure | GraphQLExecutionResultError
      >()
    })
  })
  describe('with .errors', () => {
    test('.execution: throw', async () => {
      ...
      expectTypeOf(await g.query.__typename()).toEqualTypeOf<'Query' | RequestPipelineBase.ResultFailure>()
    })
