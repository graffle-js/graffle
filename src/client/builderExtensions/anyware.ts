...
import type { RequestPipelineDefinition } from '../../requestPipeline/__.js'
...
export interface Anyware<$Arguments extends Builder.Extension.Parameters<BuilderExtensionAnyware>> {
  ...
  anyware: (
    interceptor: AnywareLib.Interceptor.InferConstructor<
      RequestPipelineDefinition // <$Arguments['context']['config']>
    >,
...
export const builderExtensionAnyware = Builder.Extension.create<BuilderExtensionAnyware>((builder, context) => {
  const properties = {
    anyware: (interceptor: AnywareLib.Interceptor.InferConstructor<RequestPipelineDefinition>) => {
      return builder({
        ...context,
        extensions: [
          ...context.extensions,
