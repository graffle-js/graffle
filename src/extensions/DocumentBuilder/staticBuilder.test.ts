...
test('type output inference', () => {
  ...
  Ts.assertEqual<Grafaid.Document.Typed.String<{ string: string | null }, never, true>>()(q1)

  const q2 = Possible.query.idNonNull(true)
  Ts.assertEqual<Grafaid.Document.Typed.String<{ idNonNull: string }, never, true>>()(q2)

  // Custom scalar
  const q3 = Possible.query.date(true)
  Ts.assertEqual<Grafaid.Document.Typed.String<{ date: string | null }, never, true>>()(q3)
  const q3b = PossibleWithScalars.query.date(true)
  Ts.assertEqual<Grafaid.Document.Typed.String<{ date: Date | null }, never, true>>()(q3b)

  // Lists
  const q5 = Possible.query.listInt(true)
  Ts.assertEqual<Grafaid.Document.Typed.String<{ listInt: Array<number | null> | null }, never, true>>()(q5)

  const q6 = Possible.query.listIntNonNull(true)
  Ts.assertEqual<Grafaid.Document.Typed.String<{ listIntNonNull: Array<number> }, never, true>>()(q6)

  const q7 = Possible.query.listListInt(true)
  Ts.assertEqual<Grafaid.Document.Typed.String<{ listListInt: Array<Array<number | null> | null> | null }, never, true>>()(q7)

  // Object with fields
  const q8 = Possible.query.object({ id: true })
  Ts.assertEqual<Grafaid.Document.Typed.String<{ object: { id:string|null } | null }, never, true>>()(q8)
