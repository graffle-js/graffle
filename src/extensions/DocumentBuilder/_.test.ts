...
Test
  ...
  .test(({ input, matrix }) => {
    const documentNormalized = GraphqlKit.Document.Object.Select.Document.createDocumentNormalizedFromQuerySelection(
      input,
    )
    const { document } = GraphqlKit.Document.Object.ToAst.toAst(documentNormalized, matrix)
