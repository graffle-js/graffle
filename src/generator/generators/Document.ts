...
export const ModuleGeneratorDocument = createModuleGenerator(
  ...
  ({ config, code }) => {
    ...
    if (mutationType) {
      ...
      code(
        `export const mutation: MutationBuilder = createStaticRootType(GraphqlKit.Schema.Runtime.OperationType.MUTATION, { sddm }) as any`,
      )
      code``
    }
    ...
    if (subscriptionType) {
      ...
      code(
        `export const subscription: SubscriptionBuilder = createStaticRootType(GraphqlKit.Schema.Runtime.OperationType.SUBSCRIPTION, { sddm }) as any`,
      )
      code``
    }
