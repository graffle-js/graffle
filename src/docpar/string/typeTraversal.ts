 ...
 * - etc.
 */
export type DecodeInlineType<$InlineType, $NamedType, $Schema extends Schema> = $InlineType extends
  [infer $First, ...infer $Rest] ? $First extends 0
    // Nullable wrapper
    ? $Rest extends [infer $Inner] ? DecodeInlineType<$Inner, $NamedType, $Schema> | null
    : ResolveNamedType<$NamedType, $Schema> | null // Terminal: nullable named type
  : $First extends 1
    // NonNull wrapper
    ? $Rest extends [infer $Inner] ? NonNullable<DecodeInlineType<$Inner, $NamedType, $Schema>>
 ...
 */
export type ApplyInlineType<$InlineType, $ResolvedType> = $InlineType extends [infer $First, ...infer $Rest]
  ? $First extends 0
    // Nullable wrapper
    ? $Rest extends [infer $Inner] ? ApplyInlineType<$Inner, $ResolvedType> | null
    : $ResolvedType | null // Terminal: nullable
  : $First extends 1
    // NonNull wrapper
    ? $Rest extends [infer $Inner] ? NonNullable<ApplyInlineType<$Inner, $ResolvedType>>
